...
def test(opt):
    print("[DEBUG] Started test function")
    ...
    if opt.use_cached_features:
        print("[DEBUG] Using cached features")
        ...
        print("[DEBUG] Computing features for input music")
        print("Computing features for input music")
        for wav_file in glob.glob(os.path.join(opt.music_dir, "*.wav")):
            print(f"[DEBUG] Processing {wav_file}")
            ...
            for idx, file in enumerate(tqdm(file_list)):
                ...
                # )[66]
                print(f"[DEBUG] Calling feature_func for {file}")
                reps, _ = feature_func(file)
                print(f"[DEBUG] Finished calling feature_func for {file}")
                # save reps
                if opt.cache_features:
    ...
    print("[DEBUG] Initializing EDGE model")
    model = EDGE(opt.feature_type, opt.checkpoint)
    print("[DEBUG] Model loaded, setting to eval")
    model.eval()
    ...
    print("[DEBUG] Starting inference rendering loop")
